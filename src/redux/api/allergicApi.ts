...
export const allergicApi = createApi({
  ...
  endpoints: (builder) => ({
    ...
    updateallergic: builder.mutation<AllergicModel, allergicEdict>({
      query: (allergic) => {
        console.log("Contenido de body:", allergic.deselectedIds); // Imprimir el contenido de allergic
        return {
          url: "update-allergic-ingredients.json",
          method: "POST",
          // body: allergic.deselectedIds,
          body: {
            user: {
              ingredient_ids: allergic.deselectedIds,
            },
          },
        };
      },
