import { NativeSyntheticEvent, ScrollView, StyleSheet, Text, TextInput, TextInputChangeEventData, TouchableOpacity, View } from 'react-native'
...
import { useUpdateaddressesMutation } from '../../redux/api/addresses';
import GoogleAddress from '../../componets/googleAddress';

type Props={
  route: RouteProp<{ params: { addressDetails: Address, selected: string }}, 'params'>
 }



const EditAddress: FunctionComponent<Props>  = ({route}) => {
  const { addressDetails, selected} = route.params;
  const id:number = addressDetails.id
  // console.log("tyme: " + addressDetails)
   const [updateaddresses,{data, isError, error, isLoading}] = useUpdateaddressesMutation();

   useEffect(() => {
    
    if(data ){
      
          toast.show('Address Update', {
            type: "success",
            placement: "bottom",
            duration: 8000,
  ...
  let [optionalAddress, SetoptionalAddress]= useState('')
    const [newAddressState, SetNewAddressState] = useState({
      ...
      apartmentSuite:addressDetails.address2,
      street: addressDetails.street_number,
      city: addressDetails.city,
      state: addressDetails.state_short,
      zip:addressDetails.zip_code,
      mobileNumber: addressDetails.phone_number,
      deliveryInstructions: addressDetails.instructions,
      securityCode: addressDetails.security_code,
      id: addressDetails.id
    })
 const toast = useToast();
//  const [addaddresses,{data, isError, error, isLoading}] = useAddaddressesMutation();
...
const onChangeApartmentSuite = (e: NativeSyntheticEvent<TextInputChangeEventData>): void => {
  const value = e.nativeEvent.text;
  SetNewAddressState(prev =>({...prev, apartmentSuite: value }));
}

const onChangeStreet = (e: NativeSyntheticEvent<TextInputChangeEventData>): void => {
  const value = e.nativeEvent.text;
  SetNewAddressState(prev =>({...prev, street: value }));
}

const onChangeCity = (e: NativeSyntheticEvent<TextInputChangeEventData>): void => {
  const value = e.nativeEvent.text;
  SetNewAddressState(prev =>({...prev, city: value }));
}

const onChangestate = (e: NativeSyntheticEvent<TextInputChangeEventData>): void => {
  const value = e.nativeEvent.text;
  SetNewAddressState(prev =>({...prev, state: value }));
}

const onChangeMobileNumber = (e: NativeSyntheticEvent<TextInputChangeEventData>): void => {
  const value = e.nativeEvent.text;
  SetNewAddressState(prev =>({...prev, mobileNumber: value }));
}

const onDeliveryInstructions = (e: NativeSyntheticEvent<TextInputChangeEventData>): void => {
  const value = e.nativeEvent.text;
  SetNewAddressState(prev =>({...prev, deliveryInstructions: value }));
}

const onsecurityCode = (e: NativeSyntheticEvent<TextInputChangeEventData>): void => {
  const value = e.nativeEvent.text;
  SetNewAddressState(prev =>({...prev, securityCode: value }));
}

const onChangezip = (e: NativeSyntheticEvent<TextInputChangeEventData>): void => {
  const value = e.nativeEvent.text;
...
const address = {
        ...
        "type_of_address": selected,
        "address1": newAddressState.address ?  newAddressState.address : optionalAddress,
        ...
        newAddressState.city + ", " + newAddressState.state,
...
const handlerAddress = async () =>{
      ...
      if(!newAddressState.firstName){
        ...
        await updateaddresses({id, address}) 
      }
  } 
  ...
  return (
    <View style={{backgroundColor: secundaryColor,  flex: 1,}}>
       <KeyboardAwareScrollView contentContainerStyle={{flex: 1}} keyboardShouldPersistTaps="handled">
          ...
          <ScrollView showsVerticalScrollIndicator={false} style={{width:ScreenWidth, marginBottom:5,}} keyboardShouldPersistTaps="handled">
              <View style={styles.viewScroll}>
                ...
                <View style={styles.inputContainer}>
                  ...
                  {/* <GoogleAddress SetNewAddressState={SetNewAddressState} newAddress={newAddress}/> */}
                  <GoogleAddress SetNewAddressState={SetNewAddressState} newAddressState={newAddressState} onChange={onChange}/>
                ...
                <View style={styles.inputContainer}>
                  ...
                  <TextInput placeholder='Type Code...' 
