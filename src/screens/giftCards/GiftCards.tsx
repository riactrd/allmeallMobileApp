...
import { useGetGiftvouchersQuery } from "../../redux/api/giftvouchersApi";

const GiftCards = () => {
  const navigation = useNavigation();
  const [listGift, setListGift] = useState([]);
  const { data } = useGetGiftvouchersQuery("");

  useEffect(() => {
    if (data) {
      setListGift(data?.data.gift_cards);
    }
  }, [data]);
        ...
        >
          <View style={styles.viewScroll}>
            {listGift.map((item, index) => (
              <NutritioItem key={index} item={item} />
            ))}
            <TouchableOpacity
              tyle={styles.containerButtom}
