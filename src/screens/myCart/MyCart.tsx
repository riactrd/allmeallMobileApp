import { FlatList, NativeSyntheticEvent, RefreshControl, ScrollView, StyleSheet, Text, TextInput, TextInputChangeEventData, TouchableOpacity, View } from 'react-native'
...
import { Bags, CartItems, Choiseofcontainer, Coupon, Expecteddeliverydate, Isgift, Membership, Nocontactdelivery, Options, Ordersummary, Total, UpdateMyCart, Usedigitalwallet, Useexistingbags } from '../../model/MyCartModel';
import { selectcartItems } from '../../redux/store';
import { ScreenWidth, Screenheight, mainColor, secundaryColor, thirdColor } from '../../componets/shared';

import { useDecreaseCartMutation, useDeleteItemCartMutation, useGetmyCartQuery, useIncreaseCartMutation } from '../../redux/api/myCartApi';
...
const MyCart = () => {
    ...
    const [expecteddelivery, setExpecteddelivery] = useState<Expecteddeliverydate>();
    const [nocontactdelivery, setNocontactdelivery] = useState<Nocontactdelivery>();
    ...
    const {data, isError, error, isLoading, refetch} = useGetmyCartQuery('bulbasaur', { refetchOnMountOrArgChange: true});
...

               {
                  ...
                  <>
                   ...
                   myItem?.map((item, index) =>(
                    ...
                    <View style={styles.textDivide}>
                      ...
                      <Text style={styles.textDivideFont}>{container?.lable}</Text>
                    </View>
                ...
                {/* {
                  container.map((item, index)=>(
                    ...
                    <View style={styles.headerContainer}>
                    <View style={styles.textDivide}>
                        <View style={[container?.checked ? styles.radiobutomActive : styles.radiobutom]}>
                            <RadioButton
                                // value={item.checked}
                                // label="Carto Base MAp"
                    ...
                    <Text style={styles.radioText}>{container?.lable}</Text>
                          ...
                          <Text style={styles.radioPriceText}>${container?.price.toFixed(2)}</Text>
                </View>

                 <View style={styles.headerContainer}>
                    <View style={styles.textDivide}>
                        <View style={[containertwo?.checked ? styles.radiobutomActive : styles.radiobutom]}>
                            <RadioButton
                                // value={item.checked}
                                // label="Carto Base MAp"
                                status={containertwo?.checked ? 'checked' : 'unchecked'}
                                onPress={onChangePlastic}
                                color={mainColor}
                                />
                        </View>
                        
                    <Text style={styles.radioText}>{containertwo?.lable}</Text>
                    </View>
                          <Text style={styles.radioPriceText}>${containertwo?.price.toFixed(2)}</Text>
                </View>
                ...
                <View style={styles.headerContainer}>
                    ...
                    <MaterialCommunityIcons name="shopping-outline" color={mainColor} style={styles.icon}/>
                      <Text style={styles.textDivideFont}>Bag</Text>
                        ...
                        <View style={styles.headerButtomTextContainer}>
                          <Text style={styles.headerButtomText}>${bagUniPrice?.price_per_quantity.toFixed(2)}</Text>
                        </View>
                ...
                <View style={styles.headerContainer}>
                    <View style={styles.textDivide}>
                        <View style={[bag?.value ? styles.radiobutomActive : styles.radiobutom]}>
                            <RadioButton
                                value="bag"
                                // label="Carto Base MAp"
                ...
                <View style={styles.headerContainer}>
                    <View style={styles.textDivide}>
                      ...
                      <Text style={styles.textDivideFont}>{isMembership?.label} ({isMembership?.price.toFixed(2)})</Text>
                    </View>
                </View>
                <View style={styles.containerChekbox}>
                      <Checkbox
                       ...
                       />
                        <Text style={styles.checkDetails}>{isMembership?.info}</Text>
                </View>
                <MyAdresses/>
                ...
                <View style={styles.picker}>
                   <Text style={styles.textHeaderPicker}>{date?.toLocaleDateString()}</Text>
                ...
                <View style={styles.headerContainer}>
                    <View style={styles.textDivide}>
                      <Text style={styles.textDivideFont}>Delivery Date</Text>
                        ...
                        <View style={styles.headerButtomTextContainer}>
                          <Text style={styles.headerButtomText}>{expecteddelivery?.selected}</Text>
                        </View>
                </View>
                <View style={styles.containerChekbox}>
                      <Checkbox
                       ...
                       />
                        <Text style={styles.checkDetails}>{nocontactdelivery?.label}</Text>
                </View>
                <View style={styles.containerChekbox}>
                      <Checkbox
                ...
                <>
                   <View style={styles.summaryContaienr}>
                     <Text style={styles.summaryText}>{ordersummary?.total_meal_amount.label}</Text>
                     <Text style={styles.summaryTextNumber}>{ordersummary?.total_meal_amount.value}</Text>
                   </View>
                   <View style={styles.summaryContaienr}>
                     <Text style={styles.summaryText}>{ordersummary?.glassware_amount.label}</Text>
                     <Text style={styles.summaryTextNumber}>{ordersummary?.glassware_amount.value}</Text>
                   </View>
                   <View style={styles.summaryContaienr}>
                     <Text style={styles.summaryText}>{ordersummary?.existing_glassware_amount.label}</Text>
                     <Text style={styles.summaryTextNumberColor}>{ordersummary?.existing_glassware_amount.value}</Text>
                   </View>
                   <View style={styles.summaryContaienr}>
                   ...
                   <View style={styles.summaryContaienrColor}>
                     <Text style={styles.summaryText}>{ordersummary?.sub_total.label}</Text>
                     <Text style={styles.summaryTextNumber}>{ordersummary?.sub_total.value}</Text>
                   </View>
                   <View style={styles.summaryContaienr}>
                     <Text style={styles.summaryText}>{ordersummary?.tax.label}</Text>
                     <Text style={styles.summaryTextNumber}>{ordersummary?.tax.value}</Text>
                   </View>
                   <View style={styles.summaryContaienr}>
                     <Text style={styles.summaryText}>{ordersummary?.delivery_charge.label}</Text>
                     <Text style={styles.summaryTextNumber}>{ordersummary?.delivery_charge.value}</Text>
                   </View>
                   <View style={styles.summaryContaienr}>
                   ...
                   <View style={styles.summaryContaienrColorTotal}>
                     <Text style={styles.summaryTextTotal}>{ordersummary?.grand_total.label}</Text>
                     <Text style={styles.summaryTextNumberTotal}>{ordersummary?.grand_total.value}</Text>
                   </View>
                   <View style={styles.summaryContaienr}>
